...
class JiugeForCauslLM:
    ...
    def generate(self, input_content, max_steps, topp_=1.0, topk_=1, temperature_=1.0):
        ...
        chat_template_content = self.tokenizer.apply_chat_template(
            conversation=[{"role": "user", "content": input_content}],
            add_generation_prompt=True,
            tokenize=False,
        )
        
        tokens = self.tokenizer.encode(chat_template_content)
        infer_task = InferTask(
            0, tokens, self.max_context_len(), temperature_, topk_, topp_, self.eos_token_id
        )
        ...
        for step_i in range(max_steps):
            ...
            else: # Capture the time for the first step (prompt processing)
                prompt_processing_time = step_time

        infer_task._kv_cache.drop(self)

        # --- Construct the dictionary with all key metrics ---
        avg_decode_time_ms = 0
        if steps > 1:
            # Calculate average decode time in milliseconds per token
            avg_decode_time_ms = total_decode_time * 1000 / (steps - 1)

        metrics = {
            'prompt_processing_time_s': prompt_processing_time,
            'avg_decode_time_ms_per_token': avg_decode_time_ms,
            'output_tokens': steps,
        }
...
def test():
    # 1. Argument Parsing
    if len(sys.argv) < 3:
        print("Usage: python script.py [--cpu|--nvidia|...] <path/to/model_dir> [n_device]")
        sys.exit(1)
    
    model_path = sys.argv[2]
    device_flag = sys.argv[1]
    device_map = { 
        "--cpu": DeviceType.DEVICE_TYPE_CPU, "--nvidia": DeviceType.DEVICE_TYPE_NVIDIA,
        "--cambricon": DeviceType.DEVICE_TYPE_CAMBRICON, "--ascend": DeviceType.DEVICE_TYPE_ASCEND,
        "--metax": DeviceType.DEVICE_TYPE_METAX, "--moore": DeviceType.DEVICE_TYPE_MOORE,
        "--iluvatar": DeviceType.DEVICE_TYPE_ILUVATAR 
    }
    device_type = device_map.get(device_flag)
    if device_type is None:
        print(f"Error: Invalid device type '{device_flag}'")
        sys.exit(1)
    ...
    # prompt_lengths now represents word count
    prompt_lengths = [32, 64, 128, 256, 512, 1024, 2048]
    output_max_tokens = 512
    results_summary = []
    
    tokenizer = model.tokenizer
    base_phrase = "hello world " # 2 words

    print(f"--- Starting inference performance test (output_max_tokens={output_max_tokens}) ---")
    for length in prompt_lengths: # length is target word count
        # Generate prompt based on word count
        num_repeats = (length // 2) + 1
        full_phrase = (base_phrase * num_repeats).strip()
        words = full_phrase.split()
        prompt = " ".join(words[:length])
        
        actual_prompt_tokens = tokenizer.encode(prompt, add_special_tokens=False)

        print(f"\n{'='*25} [Test Start] Target Prompt WORD Length: {length} {'='*25}")

        # Print input information
        print(f"  - ➡️  Input Prompt (words: {len(prompt.split())}, tokens: {len(actual_prompt_tokens)}, chars: {len(prompt)}):")
        if len(prompt) > 200:
            print(f"    '{prompt[:100]}...{prompt[-100:]}'")
        else:
            print(f"    '{prompt}'")
        
        output_content, metrics = model.generate(prompt, output_max_tokens)
        
        print(f"  - ⬅️  Model Output (tokens: {metrics['output_tokens']}):")
        print(f"    '{output_content}'")

        # Calculate and display performance metrics
        ttft = metrics['prompt_processing_time_s']
        output_tokens = metrics['output_tokens']
        avg_decode_time_ms = metrics['avg_decode_time_ms_per_token']
        tokens_per_second = 1000 / avg_decode_time_ms if avg_decode_time_ms > 0 else 0
        total_decode_time_s = (avg_decode_time_ms * (output_tokens - 1)) / 1000 if output_tokens > 1 else 0
        total_latency = ttft + total_decode_time_s

        print(f"\n  - Test performance metrics:")
        print(f"  - 首字延迟/提示处理 (TTFT) : {ttft:.4f} s")
        print(f"  - 总延迟 (Total Latency)   : {total_latency:.4f} s")
        print(f"  - 输出 Token 数         : {output_tokens} tokens")
        print(f"  - 生成速度 (Throughput)    : {tokens_per_second:.2f} tokens/s")
        print(f"  - 平均解码耗时 (Decode)    : {avg_decode_time_ms:.3f} ms/token")
        print(f"{'='*27} [Test End] {'='*28}\n")

        results_summary.append({
            "prompt_len": length, "ttft": ttft, "total_latency": total_latency,
            "output_tokens": output_tokens, "tokens_per_second": tokens_per_second
        })

    # 5. Final Summary Report
    print("\n\n--- Performance Summary Report ---")
    if results_summary:
        avg_ttft = statistics.mean([r['ttft'] for r in results_summary])
        avg_latency = statistics.mean([r['total_latency'] for r in results_summary])
        avg_throughput = statistics.mean([r['tokens_per_second'] for r in results_summary])
        print(f"测试模型: {model_path}")
        print(f"测试轮次: {len(results_summary)}")
        print("-" * 40)
        print(f"平均首字延迟 (Avg TTFT)        : {avg_ttft:.4f} s")
        print(f"平均总延迟 (Avg Latency)       : {avg_latency:.4f} s")
        print(f"平均生成速度 (Avg Throughput)    : {avg_throughput:.2f} tokens/s")
        print("-" * 40)
        print("详细数据:")
        for res in results_summary:
            print(f"  - Prompt Words: {res['prompt_len']:<4} -> TTFT: {res['ttft']:.4f}s, Latency: {res['total_latency']:.4f}s, Throughput: {res['tokens_per_second']:.2f} t/s")
    else:
        print("No successful tests to summarize.")

    # 6. Destroy Model Instance
    print("\n--- Test finished, destroying model ---")
